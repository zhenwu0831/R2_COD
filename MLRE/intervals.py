## Credits to: https://code.activestate.com/recipes/577515-an-interval-mapping-data-structure/
...
class IntervalMapping(MutableMapping):
	...
	def submapping(self, start, stop):
		...
		res = type(self)()
		res._bounds = list(bounds[lindex:rindex])
		res._values = list(values[lindex:rindex + 1])
	...
	def contains(self, start, end):
		return any([x in self for x in range(start, end)])

	def get_label(self, start, end):
		freq = Counter([self[x] for x in range(start, end) if x in self])
